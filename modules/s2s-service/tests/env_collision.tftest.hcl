...
run "env_collision_with_cognito_client_id_rejected" {
  command = plan

  variables {
    env = { COGNITO_CLIENT_ID = "evil-override" }
  }

  expect_failures = [var.env]
}

run "env_collision_with_broker_audience_rejected" {
  command = plan

  variables {
    env = { BROKER_AUDIENCE = "wrong-context" }
  }

  expect_failures = [var.env]
}

run "non_colliding_env_allowed" {
  command = plan
  ...
  assert {
    condition = anytrue([
      ...
      e.name == "LOG_LEVEL" && e.value == "info"
    ])
    error_message = "User env LOG_LEVEL must appear in container env."
  }
}
