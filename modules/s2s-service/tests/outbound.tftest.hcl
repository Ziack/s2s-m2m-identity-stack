...
variables {
  ...
  scopes                     = ["lending/write", "ledger/write"]
  ...
  outbound_audiences         = ["ledger", "deposits"]
  env                        = {}
  cedar_policies             = []
  platform = {
...
run "outbound_audiences_env_present" {
  command = plan

  assert {
    condition = anytrue([
      for e in jsondecode(aws_ecs_task_definition.this.container_definitions)[0].environment :
      e.name == "OUTBOUND_AUDIENCES" && e.value == "ledger,deposits"
    ])
    error_message = "OUTBOUND_AUDIENCES env var must equal 'ledger,deposits'."
  }
  assert {
    condition = anytrue([
      for e in jsondecode(aws_ecs_task_definition.this.container_definitions)[0].environment :
      e.name == "BROKER_TOKEN_ENDPOINT"
    ])
