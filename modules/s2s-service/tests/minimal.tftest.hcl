...
run "synthesises_cleanly" {
  command = plan

  assert {
    condition     = aws_ecs_task_definition.this.family == "loan-origination"
    error_message = "Task family must equal service_name."
  }
  assert {
    condition     = length(jsondecode(aws_ecs_task_definition.this.container_definitions)) == 1
    error_message = "With no sidecars, exactly 1 container must be defined."
  }
  assert {
    condition     = jsondecode(aws_ecs_task_definition.this.container_definitions)[0].readonlyRootFilesystem == true
    error_message = "Main container must have read-only rootfs."
  }
  assert {
    condition     = jsondecode(aws_ecs_task_definition.this.container_definitions)[0].user == "1000:1000"
    error_message = "Main container must run as uid 1000."
  }
  assert {
    condition     = length(aws_verifiedpermissions_policy.cedar) == 0
    error_message = "Expected no Cedar policies."
  }
}
