...
module "loan_origination" {
  ...
  cedar_policies = [
    {
      name      = "permit-loan-write"
      statement = file("${path.module}/policies/permit-loan-write.cedar")
    },
  ]

  tags = {
    team        = "lending-platform"
    environment = var.environment
  }
}
