...
module "internal" {
  ...
  image_uri                  = "${var.account_id}.dkr.ecr.${var.region}.amazonaws.com/${var.environment}/internal:${var.image_tag}"
  container_port             = 3000
  alb_path_pattern           = "/internal*"
  alb_listener_rule_priority = 320

  cedar_policies = [
    {
      name        = "internal"
      statement   = file("${path.module}/../policies/internal.cedar")
      description = "Actor-chain-only authz for internal endpoints"
    },
  ]
  ...
  tags = {
    example         = "migrations/from-no-auth"
    bounded_context = var.bounded_context
    environment     = var.environment
  }
}
...
output "service_url" {
  value = "https://${local.platform.alb_dns_name}/internal"
}
