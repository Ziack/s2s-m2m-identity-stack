...
module "reports" {
  ...
  image_uri                  = "${var.account_id}.dkr.ecr.${var.region}.amazonaws.com/${var.environment}/reports:${var.image_tag}"
  container_port             = 3000
  alb_path_pattern           = "/api/reports*"
  alb_listener_rule_priority = 310

  cedar_policies = [
    {
      name        = "reports"
      statement   = file("${path.module}/../policies/reports.cedar")
      description = "Casbin policy.csv translated to Cedar"
    },
  ]
  ...
  tags = {
    example         = "migrations/from-casbin-rbac"
    bounded_context = var.bounded_context
    environment     = var.environment
  }
}
...
output "service_url" {
  value = "https://${local.platform.alb_dns_name}/api/reports"
}
