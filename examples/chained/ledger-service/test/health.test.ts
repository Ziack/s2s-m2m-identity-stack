...
describe('ledger-service health endpoints', () => {
  it('GET /health is a liveness probe (always 200)', async () => {
    const res = await request(buildApp()).get('/health');
    expect(res.status).toBe(200);
    expect(res.body).toEqual({ status: 'ok' });
  });

  it('GET /health/auth returns 200 when Redis up and JWKS fresh', async () => {
