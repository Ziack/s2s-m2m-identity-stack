...
module "ledger_service" {
  ...
  image_uri                  = "${var.account_id}.dkr.ecr.${var.region}.amazonaws.com/${var.environment}/ledger-service:${var.image_tag}"
  container_port             = 3000
  alb_path_pattern           = "/api/ledger*"
  alb_listener_rule_priority = 120

  cedar_policies = [
    {
      name        = "ledger-role-checks"
      statement   = file("${path.module}/../policies/ledger.cedar")
      description = "Ledger user.roles-based authorization (terminal)"
    },
  ]
