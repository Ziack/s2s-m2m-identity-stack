...
module "receiving_service" {
  ...
  service_name               = "receiving-service"
  bounded_context            = var.bounded_context
  scopes                     = ["lending/write", "lending/read"]
  image_uri                  = "${var.account_id}.dkr.ecr.${var.region}.amazonaws.com/${var.environment}/receiving-service:${var.image_tag}"
  container_port             = 3000
  ...
  cedar_policies = [
    {
      name        = "lending-actor-chain"
      statement   = file("${path.module}/../policies/lending.cedar")
      description = "Receiving-service-owned lending policies (downstream chain)"
    },
  ]
