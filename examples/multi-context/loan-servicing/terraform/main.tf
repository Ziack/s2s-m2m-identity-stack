...
module "loan_servicing" {
  ...
  service_name    = "loan-servicing"
  bounded_context = var.bounded_context
  scopes          = ["lending/write", "lending/read"]

  # Manual ARN construction (NOT self-reference) to avoid module cycle.
  image_uri                  = "${var.account_id}.dkr.ecr.${var.region}.amazonaws.com/${var.environment}/loan-servicing:${var.image_tag}"
  container_port             = 3000
  alb_path_pattern           = "/api/loans/service*"
  alb_listener_rule_priority = 201

  cedar_policies = [
    {
      name        = "service-officer-or-servicer"
      statement   = file("${path.module}/../policies/servicing.cedar")
      description = "Permit loan-officer or loan-servicer to service"
    },
  ]
