...
module "loan_origination" {
  ...
  image_uri                  = "${var.account_id}.dkr.ecr.${var.region}.amazonaws.com/${var.environment}/loan-origination:${var.image_tag}"
  container_port             = 3000
  alb_path_pattern           = "/api/loans/originate*"
  alb_listener_rule_priority = 200

  cedar_policies = [
    {
      name        = "originate-loan-officer"
      statement   = file("${path.module}/../policies/originate.cedar")
      description = "Permit loan-officer to originate; forbid otherwise"
    },
  ]
